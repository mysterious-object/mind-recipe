...
class LlamaParent {
      ...
      if (_promptCompleters.containsKey(promptId)) {
        if (!_promptCompleters[promptId]!.isCompleted) {
          if (data.status == LlamaStatus.error) {
            _promptCompleters[promptId]!.completeError(
  ...
  Future<Uint8List> saveState(LlamaScope scope) async {
    ...
    return _stateCompleter!.future.timeout(const Duration(seconds: 30),
        onTimeout: () {
      _stateCompleter = null;
  ...
  /// Clears the active native context while keeping the loaded model ready.
  ...
  Future<void> dispose() async {
    ...
    if (_stateCompleter != null && !_stateCompleter!.isCompleted)
      _stateCompleter!.completeError("Disposed");
