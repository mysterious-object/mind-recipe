# Mind Recipe remote development

## Codespaces

1. Open the private Mind Recipe repository and choose **Code → Codespaces → Create codespace on main**.
...
`https://staging-api.mindrecipe.142.93.201.156.sslip.io`
...
curl --fail https://staging-api.mindrecipe.142.93.201.156.sslip.io/healthz
```
...
`Mind Recipe checks` runs the web build, backend tests, Flutter analyzer, and Flutter tests. `Mind Recipe Android APK` builds an ARM64 release-mode staging APK on a tag or manual dispatch and publishes `mind-recipe-android-apk` as a downloadable Actions artifact.
...
1. Download `app-release.apk` from the successful `mind-recipe-android-apk` artifact.
2. On an authorized test phone, allow installation from the source used to download the APK, then install it.
3. Launch Mind Recipe, register a test-only account, and confirm that sign-in reaches the staging API without localhost or `adb reverse`.
4. Confirm the app can load the Today/Mind Recipe surfaces and record the staging response timestamp. Do not use a real person’s account or wellness data.
...
- If the API is unhealthy, inspect `kubectl --context do-k3s -n mind-recipe-staging get pods,events` and rollout status before changing source.
